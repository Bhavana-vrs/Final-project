...
<title>list of_appointments</title>
</head>
<style>ul{
font-size:24px;
}
...
<th class="text-center text-dark active font-weight-bold"><h3>Specilization</h3></th>
				   ...
				   while($drow = mysqli_fetch_array($edit_run_doctor_profile_query))
				     {
                        ?>
                        <tr>
                       <th><h3><?php  echo "<a href='../detail.php?s_id={$drow['s_id']}'>{$drow['dfname']}{$drow['dlname']}</a>";  ?></h3></th>
                       <th><h3><?php echo $drow['specialist'];  	?></h3></th>
...
<ul class="pager font-weight-bold text-monospace active ">
  <li class="previous bg-dark "><a href="view_admin_home_page.php">Home</a></li>
</ul></div>
