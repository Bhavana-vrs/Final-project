...
<th><h3>Status</th>
<th><h3><input class="alert-success" type ="submit" value="Approve" name="submit"></h3></th>
...
//$d_email = $_SESSION['email'];
$pid=$_POST['pid'];
$choose_doctor_schedule_query="select * from doctor join schedule on schedule.d_id=doctor.id join appointment on appointment.sid=schedule.s_id  join user on appointment.pid=user.id where doctor.email='$d_email' and appointment.permission='pending' ";
...
//$d_email = $_SESSION['email'];
$pid=$_POST['pid'];
$choose_doctor_schedule_query="select * from doctor join schedule on schedule.d_id=doctor.id join appointment on appointment.sid=schedule.s_id  join user on appointment.pid=user.id where doctor.email='$d_email' and appointment.permisii
='pending';";
	$run_doctor_schedule_query=mysqli_query($db,$choose_doctor_schedule_query);
...
$edit_doctor_profile_query="select * from doctor join schedule on schedule.d_id=doctor.id join appointment on appointment.sid=schedule.s_id  join user on appointment.pid=user.id where doctor.email='$d_email' and appointment.permission='Pending'";
				   ...
				   while($drow = mysqli_fetch_array($edit_run_doctor_profile_query))
				     {
                        ...
                        <tr>
                           ...
                           <th> <h3><?php   echo  "<a href='../user_detail.php?id={$drow['id']}'>{$drow['f_name']}{$drow['l_name']}</a>";?></h3></th>
...
<ul class="pager font-weight-bold text-monospace">
  <li class="previous "><a href="doctor_home_page.php">Home</a></li>
